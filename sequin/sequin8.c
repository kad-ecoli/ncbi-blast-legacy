...

{
  ...
  if (curr != NULL && curr->choice == 1) {
    ...
    if (StringICmp (str, "Features") == 0) {
      status = 1;
    } else if (StringICmp (str, "Organism") == 0) {
      status = 2;
    }
    SetValue (rdp->featOrSource, status);
  }
}
...

{
  ...
  rdp->featOrSource = HiddenGroup (p, 3, 0, NULL);
  RadioButton (rdp->featOrSource, "Features");
  RadioButton (rdp->featOrSource, "Organism");
  SetValue (rdp->featOrSource, 1);
