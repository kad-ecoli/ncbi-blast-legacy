...
static void AddIndexerCommentCallback (BioseqPtr bsp, Pointer data)
{
  ...
  } else if (a->add_unverified_org) {
    uop = AddUnverifiedUserObjectToBioseqParent(bsp);
    if (uop != NULL) {
...
static const Int4 kUnverifiedSource = 3;
static const Int4 kChimeraComment = 4;
...
static void MakeSpecialGapSequencesMenu (MenU m, BaseFormPtr bfp)
{
  ...
  SetObjectExtra (i, bfp, NULL);
  i = CommandItem (s, "Trim CDS-mRNA for Terminal Ns", AdjustCodingRegionsWithTerminalNs);
  SetObjectExtra (i, bfp, NULL);
